...
const fetchStudentDashboardData = async (setCounts, setLoading, setError) => {
  try {
    setLoading(true);
    const response = await fetch('http://localhost:5000/api/dashboard'); // Assuming this returns all counts

    if (!response.ok) {
      throw new Error(`Error ${response.status}: Failed to fetch student dashboard data`);
    }
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    fetchStudentDashboardData(setCounts, setLoading, setError);
  }, []);
          ...
          >
            ...
            <Box sx={{ marginTop: 2 }}>
              <div className="dashboard-links">
                <ul>
                  <li>
                    <Link to="/student/view-teachers">View Teachers</Link>
                  </li>
                  <li>
                    <Link to="/student/view-subjects">View Subjects</Link>
                  </li>
                  <li>
                    <Link to="/student/view-events">View Events</Link>
                  </li>
                  <li>
                    <Link to="/student/view-marks">View Marks</Link>
                  </li>
                  <li>
